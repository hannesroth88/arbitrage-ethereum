...
export async function getBestCrossedMarket(crossedMarkets: Array<EthMarket>[], tokenAddress: string): Promise<CrossedMarketDetails | undefined> {
  ...
  for (const crossedMarket of crossedMarkets) {
    ...
    const buyWETHReserves = bigNumberToDecimal(buyWETHMarket.getBalance(WETH_ADDRESS_FINAL));
    const sellWETHReserves = bigNumberToDecimal(sellWETHMarket.getBalance(WETH_ADDRESS_FINAL));
    ...
    if (!buyTokenReserves || !sellTokenReserves) {
      try {
        ...
        console.error(`
          marketAWETHReserves: ${buyWETHMarket.getBalance(WETH_ADDRESS_FINAL)}\n
          marketATokenReserves: ${buyWETHMarket.getBalance(tokenAddress)}\n
          marketBWETHReserves: ${sellWETHMarket.getBalance(WETH_ADDRESS_FINAL)}\n
    ...
    try {
      ...
      const numeratorA = kSell ** 0.5 * buyTokenReserves;
      const numeratorB = gamma ** -1 * (kBuy ** 0.5 * sellTokenReserves);
      const denominator = kBuy ** 0.5 + kSell ** 0.5;
      ...
      if ((bestCrossedMarket === undefined || profit > bestCrossedMarket.profit) && profit && profit > 0.001) {
...
async function _evaluateMarketsForToken(tokenAddress: string, markets: Array<EthMarket>): Promise<CrossedMarketDetails | undefined> {
  ...
  if (bestCrossedMarket !== undefined && bestCrossedMarket.profit > 0.001) {
    return bestCrossedMarket;
  }
}
...
export class Arbitrage {
  ...
  static async printCrossedWETHMarket(crossedMarket: CrossedMarketDetails): Promise<void> {
    ...
    const tokenAddress =
      crossedMarket.buyFromMarket.tokens[0] === WETH_ADDRESS_FINAL ? crossedMarket.buyFromMarket.tokens[1] : crossedMarket.buyFromMarket.tokens[0];
    const WETH = await Token.getToken(WETH_ADDRESS_FINAL);
    const token = await Token.getToken(tokenAddress);
  ...
  async takeCrossedMarkets(bestCrossedMarkets: CrossedMarketDetails[], blockNumber: number, minerRewardPercentage: number): Promise<void> {
    for (const bestCrossedMarket of bestCrossedMarkets) {
      ...
      const inter = bestCrossedMarket.buyFromMarket.getTokensOut(WETH_ADDRESS_FINAL, bestCrossedMarket.tokenAddress, bigNumberVolume);
