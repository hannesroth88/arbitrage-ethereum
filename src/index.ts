...
import { FACTORY_ADDRESSES, FACTORY_ADDRESSES_GOERLI } from "./addresses";
...
let FACTORY_ADDRESSES_CHOOSEN: any[];
if (process.env.NODE_ENV == "mainnet") {
  console.log("Starting on ETHEREU MAINNET");
  ETHEREUM_RPC_URL = process.env.ETHEREUM_RPC_URL;
  PRIVATE_KEY = process.env.PRIVATE_KEY;
  BUNDLE_EXECUTOR_ADDRESS = process.env.BUNDLE_EXECUTOR_ADDRESS;
  FACTORY_ADDRESSES_CHOOSEN = FACTORY_ADDRESSES;
} else {
  console.log("Starting on TESTNET");
  ETHEREUM_RPC_URL = process.env.ETHEREUM_RPC_URL_GOERLI;
  PRIVATE_KEY = process.env.PRIVATE_KEY_GOERLI;
  BUNDLE_EXECUTOR_ADDRESS = process.env.BUNDLE_EXECUTOR_ADDRESS_GOERLI;
  FACTORY_ADDRESSES_CHOOSEN = FACTORY_ADDRESSES_GOERLI;
}
...
async function main() {
   ...
   */
  await UniswappyV2EthPair.getUniswapMarketsByToken(provider, FACTORY_ADDRESSES_CHOOSEN);
  ...
  console.log(
    `Found ${Object.keys(markets.marketsByToken).length} token across ${
      markets.filteredMarketPairs.length
    } pools with sufficient liquidity to Arb.\n\n\n`
  );
