...
export class UniswappyV2EthPair extends EthMarket {
  ...
  static async processPair(pair: Array<string>, factoryAddress: string, provider: providers.JsonRpcProvider): Promise<void> {
  ...
  static async getUniswapMarketsByToken(provider: providers.JsonRpcProvider, factoryAddresses: Array<string>): Promise<void> {
    ...
    await Promise.all(_.map(factoryAddresses, (factoryAddress) => UniswappyV2EthPair.getUniswappyMarkets(provider, factoryAddress)));
